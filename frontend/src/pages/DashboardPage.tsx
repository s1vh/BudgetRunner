  ...
  return (
    <div className="page-enter grid gap-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <StatCard label={t('dashboard.balance')} value={formatMoney(dashboard.balanceMinor, dashboard.currency)} detail={t('dashboard.balanceDetail')} icon={WalletCards} trend="up" tone="cyan" />
        <StatCard label={t('dashboard.budgetRemaining')} value={formatMoney(dashboard.budgetRemainingMinor, dashboard.currency)} detail={t('dashboard.closureInDays', { days: 15 })} icon={Radar} tone="magenta" />
        <StatCard label="SynthCoins" value={formatNumber(dashboard.progress.synthcoins)} detail={t('dashboard.synthDetail')} icon={Coins} tone="purple" />
        <SynthCard className="flex min-h-40 flex-col items-center justify-center gap-4 p-5" tone="cyan"><Zap className="size-7 text-neon-cyan drop-shadow-[0_0_10px_#00ffff]" /><Button className="w-full" icon={Plus} onClick={() => setFormOpen(true)}>{t('dashboard.addExpense')}</Button><p className="text-center font-mono text-[10px] text-text-muted">{t('dashboard.quickEntry')}</p></SynthCard>
      </div>
