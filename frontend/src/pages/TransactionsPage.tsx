...
export function TransactionsPage() {
  ...
  return (
    <div className="page-enter grid gap-6">
      <PageHeader eyebrow={t('transactions.eyebrow')} title={t('transactions.title')} description={t('transactions.description')} icon={WalletCards} actions={<Button icon={Plus} onClick={() => setEditing('new')}>{t('transactions.new')}</Button>} />
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <SynthCard className="p-4" tone="magenta"><p className="font-mono text-[10px] text-text-muted uppercase">{t('transactions.filteredExpense')}</p><strong className="mt-2 block font-display text-xl text-neon-magenta tabular">{formatMoney(expenses, data.profile.primaryCurrency)}</strong></SynthCard>
        <SynthCard className="p-4" tone="cyan"><p className="font-mono text-[10px] text-text-muted uppercase">{t('transactions.filteredIncome')}</p><strong className="mt-2 block font-display text-xl text-neon-cyan tabular">{formatMoney(income, data.profile.primaryCurrency)}</strong></SynthCard>
        <SynthCard className="p-4" tone="purple"><p className="font-mono text-[10px] text-text-muted uppercase">{t('transactions.visibleBalance')}</p><strong className="mt-2 block font-display text-xl text-tertiary tabular">{formatMoney(income - expenses, data.profile.primaryCurrency)}</strong></SynthCard>
      </div>
