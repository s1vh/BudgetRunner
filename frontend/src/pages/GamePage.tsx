...
export function GamePage() {
  ...
  return (
    <div className="page-enter grid gap-6">
      <PageHeader eyebrow={t('game.eyebrow')} title={t('game.title')} description={t('game.description')} icon={Gamepad2} />
      ...
      {tab === 'summary' && <section className="grid gap-4">
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {[
            { label: t('common.level', { level: '' }).trim(), value: game.progress.level, icon: Sparkles, color: 'text-neon-cyan' },
            { label: 'SynthCoins', value: formatNumber(game.progress.synthcoins), icon: Coins, color: 'text-sunset' },
            { label: t('game.activePower'), value: game.progress.activePower, icon: Zap, color: 'text-neon-magenta' },
            { label: t('game.familyBonus'), value: `+${game.progress.familyBonusPower}`, icon: Shield, color: 'text-tertiary' },
          ].map(({ label, value, icon: Icon, color }) => <SynthCard key={label} className="p-5"><div className="flex items-center justify-between"><p className="font-mono text-[10px] tracking-wider text-text-muted uppercase">{label}</p><Icon className={`size-4 ${color}`} /></div><strong className={`mt-3 block font-display text-3xl tabular ${color}`}>{value}</strong></SynthCard>)}
        </div>
        <SynthCard className="p-5 sm:p-6" tone="purple">
      ...
      {tab === 'store' && <section>
        ...
        {offerCount > 0 ? <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{game.offers.map((offer) => <ModuleCard key={offer.id} module={offer.module} footer={<div className="flex items-center justify-between gap-3"><div><span className="block font-mono text-[9px] text-text-muted">{t('game.netCost')}</span><strong className="font-display text-lg text-sunset">{offer.netCost} SC</strong></div><Button className="px-3 text-[10px]" variant={offer.minLevel > game.progress.level ? 'ghost' : 'cyan'} disabled={offer.minLevel > game.progress.level} onClick={(event) => { event.stopPropagation(); setSelectedOffer(offer) }}>{offer.minLevel > game.progress.level ? t('common.level', { level: offer.minLevel }) : t('game.buy')}</Button></div>} />)}</div> : <SynthCard className="p-8 text-center"><PackageOpen className="mx-auto mb-3 size-8 text-tertiary" /><h3 className="font-display text-sm uppercase">{t('game.rotationComplete')}</h3><p className="mt-2 text-sm text-text-muted">{t('game.rotationCompleteDesc')}</p></SynthCard>}
      </section>}

      {tab === 'repairs' && <section><div className="mb-4 rounded-lg border border-tertiary/20 bg-tertiary/5 p-4 text-sm text-text-muted"><BatteryCharging className="mr-2 inline size-4 text-tertiary" />{t('game.repairsInfo')}</div><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{repairable.map((module) => { const cost = module.repairCost ?? 0; return <ModuleCard key={module.instanceId} module={module} footer={<div className="flex items-center justify-between gap-3"><span className="font-mono text-xs text-sunset">{cost} SC</span><Button variant="purple" icon={Wrench} className="px-3 text-[10px]" loading={actionBusy === module.instanceId} onClick={async (event) => { event.stopPropagation(); setActionBusy(module.instanceId); try { await repairModule(module.instanceId); setNotice(t('game.repairConfirmed', { name: module.name })) } catch (caught) { setNotice(caught instanceof Error ? caught.message : t('game.repairFailed')) } finally { setActionBusy(null) } }}>{t('game.repair')}</Button></div>} />})}</div></section>}
