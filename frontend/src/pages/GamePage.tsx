...
import {
  ...
  useCyberdeckQuery,
  useFamilyBonusesQuery,
  useGameHistoryQuery,
  useGameSummaryQuery,
} from '@/app/dataQueries'
...
export function GamePage() {
  ...
  const cyberdeckQuery = useCyberdeckQuery(visibleTab === 'deck' || visibleTab === 'repairs')
  ...
  const repairable = useMemo(() => cyberdeckQuery.data?.filter((module) => module.energy > 0 && module.energy < 100) ?? [], [cyberdeckQuery.data])
  ...
  const preloadStore = () => {
    preloadGameStorePanel()
    void queryClient.prefetchQuery(storeOffersQueryOptions)
  }
  ...
  return (
    <div className="page-enter grid gap-6">
      ...
      {visibleTab === 'summary' && (!gameProgress || !familyBonuses || summaryQuery.isPending || familyBonusesQuery.isPending || summaryQuery.isError || familyBonusesQuery.isError
      ...
      {visibleTab === 'deck' && (!cyberdeck || cyberdeckQuery.isPending || cyberdeckQuery.isError
        ...
        : <section className="grid gap-4" data-tour="game-deck"><SynthCard className="p-3 pr-14 sm:p-5 sm:pr-16" helpKey="help.game.cyberdeck"><div className="mb-4 flex items-center justify-between"><div><h2 className="font-display text-sm font-bold uppercase">{t('game.diagramTitle')}</h2><p className="mt-1 text-xs text-text-muted">{t('game.diagramHint')}</p></div><Badge tone="cyan">{cyberdeck.filter((module) => module.state === 'equipped').length}/10 {t('common.online')}</Badge></div><CyberdeckDiagram modules={cyberdeck} selectedId={selectedModule?.instanceId} onSelect={setSelectedModule} /></SynthCard></section>)}
