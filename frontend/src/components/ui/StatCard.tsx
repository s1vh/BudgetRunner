...
export function StatCard({ label, value, detail, icon: Icon, tone = 'cyan', trend, footer, helpKey }: { label: string; value: string; detail?: string; icon: LucideIcon; tone?: 'cyan' | 'magenta' | 'purple'; trend?: 'up' | 'down'; footer?: ReactNode; helpKey?: TranslationKey }) {
  const colors = { cyan: 'text-neon-cyan', magenta: 'text-neon-magenta', purple: 'text-tertiary' }
  return (
    <SynthCard className="flex min-h-40 flex-col justify-between p-5 pr-14" tone={tone} interactive helpKey={helpKey}>
