  ...
  return (
    <div className="relative overflow-x-auto rounded-xl border border-outline-soft/50 bg-void/72 p-2 shadow-[inset_0_0_60px_rgba(139,0,255,.12)]">
      <div className="relative min-w-[820px] overflow-hidden rounded-lg">
        ...
        <svg viewBox="0 0 860 630" className="relative z-10 min-w-[820px]" role="group" aria-label="Cyberdeck con diez slots conectados">
          <defs>
          ...
          <g opacity="0.7">
            ...
            <text x="430" y="312" textAnchor="middle" fill="#00FFFF" fontFamily="Orbitron" fontSize="8" letterSpacing="2">WRIST</text>
            <text x="430" y="326" textAnchor="middle" fill="#F4F4F9" fontFamily="Orbitron" fontSize="9" letterSpacing="1">CORE</text>
          </g>
          ...
          {modules.map((module, index) => {
            ...
            return (
              <g key={module.instanceId} role="button" tabIndex={0} aria-label={`${module.slotLabel}: ${module.name}, Energy ${module.energy}%`} onClick={() => onSelect(module)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') onSelect(module) }} className="cursor-pointer outline-none">
                ...
                <text x={x + 12} y={y + 27} fill="#9B91AD" fontFamily="Courier Prime" fontSize="8" letterSpacing="1.4">{module.slot.toUpperCase()} // SLOT {String(index + 1).padStart(2, '0')}</text>
                ...
                {module.state === 'destroyed' && <><path d={`M ${x + 5} ${y + 5} L ${x + 165} ${y + 89}`} stroke="#FF6E84" strokeWidth="2" strokeOpacity="0.72" /><text x={x + 85} y={y + 88} textAnchor="middle" fill="#FF6E84" fontFamily="Courier Prime" fontSize="7">SIGNAL LOST</text></>}
              </g>
            )
          })}
        </svg>
        <div className="pointer-events-none absolute bottom-3 left-1/2 z-20 -translate-x-1/2 rounded border border-neon-cyan/15 bg-void/70 px-3 py-1 font-mono text-[8px] tracking-[0.18em] text-neon-cyan/65 uppercase backdrop-blur">OpenGL wireframe · passive telemetry</div>
      </div>
    </div>
  )
}
