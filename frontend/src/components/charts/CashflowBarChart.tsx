  ...
  return (
    <div>
      <div className="mb-4 flex flex-wrap gap-4 font-mono text-[10px] tracking-wider uppercase">
        <span className="flex items-center gap-2 text-text-muted"><i className="size-2 rounded-full bg-neon-cyan shadow-[0_0_8px_#00ffff]" />Ingresos</span>
        <span className="flex items-center gap-2 text-text-muted"><i className="size-2 rounded-full bg-neon-magenta shadow-[0_0_8px_#ff007f]" />Gastos</span>
      </div>
      <div className="overflow-x-auto pb-2">
        <svg viewBox="0 0 700 260" className="min-w-[620px]" role="img" aria-label="Gráfico de ingresos y gastos mensuales">
          <defs>
          ...
          {data.map((point, index) => {
            ...
            return (
              <g key={point.label} tabIndex={0} aria-label={`${point.label}: ingresos ${formatMoney(point.incomeMinor, currency)}, gastos ${formatMoney(point.expenseMinor, currency)}`}>
                <rect x={centerX - barWidth - 2} y={baseline - incomeHeight} width={barWidth} height={incomeHeight} rx="3" fill="url(#income-bars)" stroke="#00ffff" strokeOpacity="0.5"><title>Ingresos: {formatMoney(point.incomeMinor, currency)}</title></rect>
                <rect x={centerX + 2} y={baseline - expenseHeight} width={barWidth} height={expenseHeight} rx="3" fill="url(#expense-bars)" stroke="#ff007f" strokeOpacity="0.5"><title>Gastos: {formatMoney(point.expenseMinor, currency)}</title></rect>
                <text x={centerX} y="244" fill="#9b91ad" fontFamily="Courier Prime" fontSize="10" textAnchor="middle">{point.label}</text>
              </g>
            )
          })}
        </svg>
