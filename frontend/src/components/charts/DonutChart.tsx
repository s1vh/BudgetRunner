...
export function DonutChart({ data, currency = 'EUR', title = 'Distribución de gastos' }: { data: CategoryDistribution[]; currency?: string; title?: string }) {
  ...
  return (
    <div className="grid min-h-64 items-center gap-7 md:grid-cols-[minmax(180px,0.8fr)_1.2fr]" role="img" aria-label={`${title}. Total ${formatMoney(total, currency)}.`}>
        ...
        <svg viewBox="0 0 120 120" className="size-full -rotate-90 overflow-visible" aria-hidden="true">
          ...
          {segments.map(({ item, length, offset }) => {
            return (
              ...
              >
                <title>{item.category}: {formatMoney(item.amountMinor, currency)} ({item.percentage}%)</title>
              </circle>
            )
          })}
        </svg>
        <div className="absolute inset-0 grid place-items-center text-center">
          <div><strong className="font-display text-xl text-text-glow tabular">{formatMoney(total, currency)}</strong><span className="mt-1 block font-mono text-[10px] tracking-widest text-text-muted uppercase">Gastado</span></div>
        </div>
      </div>
      <ul className="grid gap-3">
        {data.map((item) => (
          <li key={item.category} className="grid grid-cols-[auto_1fr_auto] items-center gap-3 rounded-lg border border-white/6 bg-white/[0.025] px-3 py-2.5">
            ...
            <span className="min-w-0 truncate text-sm text-text-muted">{item.category}</span>
