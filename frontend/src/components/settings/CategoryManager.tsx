...
const iconOptions = [
  ...
  ['utensils', 'icon.food'],
  ['car', 'icon.transport'],
  ['building', 'icon.housing'],
  ['cpu', 'icon.technology'],
  ['gamepad', 'icon.leisure'],
  ['heart-pulse', 'icon.health'],
  ['radio', 'icon.subscriptions'],
...
export function CategoryManager() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (editingId) {
        ...
        setNotice(t('categories.updated', { name: input.name }))
      } else {
        await createCategory(input)
        setNotice(t('categories.created', { name: input.name }))
      }
      setEditorOpen(false)
      setEditingId(null)
      setDraft(emptyDraft)
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : t('categories.operationFailed'))
    } finally {
      setBusy(false)
    }
  }
  ...
  async function remove(category: Category) {
    ...
    try {
      ...
      setNotice(t('categories.deleted', { name: categoryLabel(category, td) }))
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : t('categories.operationFailed'))
    } finally {
      setBusy(false)
    ...
    <SynthCard className="p-5 sm:p-6">
      <div className="mb-4 flex items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-display text-sm font-bold uppercase">{t('categories.title')}</h2>
        </div>
      ...
      {editorOpen && (
        <form className="mb-4 grid gap-4 rounded-lg border border-neon-cyan/20 bg-neon-cyan/[0.035] p-4" onSubmit={(event) => void submit(event)}>
          <div className="flex items-center justify-between gap-3">
            ...
            <button type="button" className="rounded p-1 text-text-muted transition hover:bg-white/5 hover:text-white" onClick={closeEditor} aria-label={t('categories.closeEditor')}>
              <X className="size-4" />
            </button>
          </div>
          <div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_minmax(0,0.8fr)_5rem]">
            <Field label={t('categories.name')} htmlFor="category-name" required>
              <Input id="category-name" name="name" value={draft.name} maxLength={80} autoFocus onChange={(event) => setDraft((current) => ({ ...current, name: event.target.value }))} placeholder={t('categories.example')} />
            </Field>
            <Field label={t('categories.icon')} htmlFor="category-icon">
              <Select id="category-icon" name="icon" value={draft.icon} onChange={(event) => setDraft((current) => ({ ...current, icon: event.target.value }))}>
                {iconOptions.map(([value, labelKey]) => <option key={value} value={value}>{t(labelKey)}</option>)}
              </Select>
            </Field>
            <Field label={t('categories.color')} htmlFor="category-color">
          ...
          <div className="flex flex-wrap justify-end gap-2">
            <Button type="button" variant="ghost" onClick={closeEditor} disabled={busy}>{t('common.cancel')}</Button>
            <Button type="submit" icon={Save} loading={busy}>{editingId ? t('common.save') : t('categories.create')}</Button>
          </div>
        </form>
      )}
          ...
          <div key={category.id} className="rounded-lg border border-white/7 bg-white/[0.025] p-3">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <span className="flex min-w-0 items-center gap-3">
                ...
                <span className="truncate text-sm">{categoryLabel(category, td)}</span>
              </span>
              <span className="flex items-center gap-1">
                <Button type="button" variant="ghost" className="min-h-8 px-2 text-[10px]" icon={Pencil} onClick={() => openEdit(category)} disabled={busy}>{t('common.edit')}</Button>
                <Button type="button" variant="ghost" className="min-h-8 px-2 text-[10px] text-neon-magenta" icon={Trash2} onClick={() => setDeleteId(category.id)} disabled={busy}>{t('common.delete')}</Button>
              </span>
            </div>
            {deleteId === category.id && (
              <div className="mt-3 flex flex-wrap items-center justify-between gap-3 border-t border-neon-magenta/15 pt-3">
                <p className="text-xs text-text-muted">{t('categories.archiveWarning')}</p>
                <span className="flex gap-2">
                  <Button type="button" variant="ghost" className="min-h-8 px-3 text-[10px]" onClick={() => setDeleteId(null)} disabled={busy}>{t('common.cancel')}</Button>
                  <Button type="button" variant="magenta" className="min-h-8 px-3 text-[10px]" loading={busy} onClick={() => void remove(category)}>{t('categories.confirmDelete')}</Button>
                </span>
              </div>
            )}
          </div>
