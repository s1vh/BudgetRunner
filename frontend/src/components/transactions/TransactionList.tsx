...
export function TransactionList({ transactions, limit, onSelect }: { transactions: FinancialTransaction[]; limit?: number; onSelect?: (transaction: FinancialTransaction) => void }) {
        ...
        return (
          <li key={transaction.id}>
            <button type="button" onClick={() => onSelect?.(transaction)} className="grid w-full grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 rounded-lg px-1 py-3 text-left transition hover:bg-white/[0.035] sm:px-2">
              ...
              <span className="min-w-0"><span className="flex items-center gap-2"><strong className="truncate text-sm text-text-glow">{transaction.concept}</strong>{transaction.lockedByReward && <LockKeyhole className="size-3 text-sunset" aria-label="Bloqueada por recompensa" />}</span><small className="mt-0.5 flex items-center gap-1.5 font-mono text-[10px] text-text-muted"><span className="truncate">{transaction.categoryName}</span><span aria-hidden="true">·</span>{transaction.status === 'scheduled' && <CalendarClock className="size-3 text-tertiary" />}{formatShortDate(transaction.occurredAt)}</small></span>
